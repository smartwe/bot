...
class Chat:
    ...
    async def send_audio(self, data, w, h):
        path, key, url = upload(data, "audio/mp3", self.author_id)
        return await self.channel.forward_chat("", json.dumps({
            "thumbnailUrl": url,
            "thumbnailHeight": w,
            "thumbnailWidth": h,
    ...
    async def send_audio_by_path(self, path):
        with open(path, "rb") as f:
            data = f.read()

        return await self.send_audio(data, 480, 480)
